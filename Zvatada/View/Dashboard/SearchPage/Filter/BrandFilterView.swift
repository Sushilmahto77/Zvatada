...
struct BrandFilterView: View {
    ...
    @State var BrandResponseModel:BrandModel
        ...
        ZStack{
            ...
            VStack{
                ...
                LazyVGrid(columns: columns, spacing: 0) {
                    ForEach(BrandResponseModel.data){item in
                        //VStack{
                        Button(action: {
...
//                           isChecked = false
            ...
            .onAppear{
                brandApi(catid: "\(catID)")
            }
    }
    func brandApi(catid:String){
        let parameters = [
        ...
        NetworkManager.postRequestWithUserTokenFixUrlRetunData(remainingUrl: "brands", parameters: parameters, Token: "\(NetworkManager.getGeneralToken())") { responseData in
            ...
            if swiftyJsonVar["status"].stringValue == "200"{
                let postResponse = try! JSONDecoder().decode(BrandModel.self, from:responseData)
                BrandResponseModel = postResponse
                print(" subCategory List Response:\(BrandResponseModel)")
            }else{
...
struct BrandFilterView_Previews: PreviewProvider {
    static var previews: some View {
        BrandFilterView(catID: .constant(""), sortValue: .constant(""), searchText: "", BrandResponseModel: BrandModel(status: "", msg: "", data: []), brandIDis: .constant(""), sizeIDis: .constant(""))
    }
}
